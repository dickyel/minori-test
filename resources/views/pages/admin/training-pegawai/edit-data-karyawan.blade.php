...
<div id="content" class="p-4 p-md-5 pt-5">
    ...
    <form action="{{ route('data-karyawan-update', $item->id) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT') 
                ...
                <div class="form-group mb-4">
                    ...
                    <select name="id" id="id" class="form-control">
                        @foreach($trainings as $training)
                            <option value="{{ $training->id }}" {{ $training->id == $item->id ? 'selected' : '' }}>
                                {{ $training->jenis }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group mb-4">
                    ...
                    <select name="pegawai_id" id="pegawai_id" class="form-control">
                        @foreach($pegawais as $pegawai)
                            <option value="{{ $pegawai->id }}" {{ $pegawai->id == $item->pegawai_id ? 'selected' : '' }}>
